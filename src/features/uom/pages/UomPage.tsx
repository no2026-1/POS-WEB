...
import { uomApi } from '../services/uom.service'
import type { Uom, CreateUomPayload } from '../types/uom.types'
...
function UomPanel({
  ...
  const [form, setForm] = useState<CreateUomPayload>({
    uomCode: uom?.uomCode ?? genDocNo('UOM'),
    uomName: uom?.uomName ?? '',
    remarks: uom?.remarks ?? '',
  })
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      const payload = clean(form) as CreateUomPayload
      return isEdit ? uomApi.update(uom!.id, payload) : uomApi.create(payload)
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => uomApi.delete(uom!.id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['uoms'] })
  ...
  return (
    <div className="w-96 bg-white border-l border-gray-200 flex flex-col h-full shadow-xl">
      ...
      <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">{t('uom.code')} *</label>
          {isEdit ? (
            <input className="input" value={form.uomCode} disabled />
          ) : (
            <div className="input bg-gray-50 text-gray-500 font-mono text-sm">{form.uomCode}</div>
          )}
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">{t('uom.name')} *</label>
          <input className="input" value={form.uomName}
            onChange={(e) => setForm(f => ({ ...f, uomName: e.target.value }))}
            placeholder="Pieces" />
        </div>
        <div>
        ...
        {!confirmDelete ? (
          <>
            <button onClick={() => saveMutation.mutate()}
              disabled={saveMutation.isPending || !form.uomCode || !form.uomName}
...
export default function UomPage() {
  ...
  const [panel, setPanel] = useState<{ open: boolean; uom: Uom | null }>({ open: false, uom: null })
  const [toast, setToast] = useState<{ message: string; type: 'success' | 'error' } | null>(null)

  const { data, isLoading } = useQuery({
    queryKey: ['uoms'],
    queryFn: () => uomApi.getAll({ limit: 100 }),
  })

  const allUoms = data?.data.data.data ?? []

  const uoms = useMemo(() => {
    if (!search.trim()) return allUoms
    const q = search.toLowerCase()
    return allUoms.filter((u) =>
      [u.uomCode, u.uomName].some((v) => v && String(v).toLowerCase().includes(q)),
    )
  }, [allUoms, search])
    ...
    <div className="flex h-full -m-6">
      <div className="flex flex-col flex-1 min-w-0">
        <div className="bg-gradient-to-r from-slate-800 via-slate-700 to-slate-600 px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-white font-bold text-lg">{t('uom.title')}</h1>
            <p className="text-slate-300 text-xs mt-0.5">{uoms.length} / {allUoms.length} {t('common.items')}</p>
          </div>
          <button onClick={() => setPanel({ open: true, uom: null })}
        ...
        <div className="flex-1 overflow-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-500 text-left sticky top-0">
              <tr>
                <th className="px-5 py-3 font-medium">{t('uom.code')}</th>
                <th className="px-5 py-3 font-medium">{t('uom.name')}</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr key={u.id} onClick={() => setPanel({ open: true, uom: u })}
                  className={`cursor-pointer transition-colors hover:bg-blue-50 ${panel.uom?.id === u.id ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''}`}>
                  <td className="px-5 py-3 font-mono font-semibold text-slate-700">{u.uomCode}</td>
                  <td className="px-5 py-3 font-medium text-gray-800">{u.uomName}</td>
                  <td className="px-5 py-3 text-gray-500">{u.remarks ?? '-'}</td>
      ...
      {panel.open && (
        <UomPanel uom={panel.uom}
          onClose={() => setPanel({ open: false, uom: null })} onToast={showToast} />
      )}
