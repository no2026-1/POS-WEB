...
import { stockAdjustApi } from '../services/stock-adjust.service'
import { productApi } from '@/features/product/services/product.service'
import { uomApi } from '@/features/uom/services/uom.service'
import type { CreateStockAdjustLinePayload } from '../types/stock-adjust.types'
...
const EMPTY_LINE: CreateStockAdjustLinePayload = { lineNo: 1, productId: 0, uomId: 0, qty: 1 }

export default function StockAdjustDetailPage() {
  ...
  const [lineForm, setLineForm] = useState<CreateStockAdjustLinePayload>({ ...EMPTY_LINE })
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['stock-adjust', adjId],
    queryFn: () => stockAdjustApi.getById(adjId),
  })
  ...
  const confirmMutation = useMutation({
    mutationFn: () => confirmAction === 'confirm'
      ? stockAdjustApi.confirm(adjId)
      : stockAdjustApi.cancel(adjId),
    onSuccess: () => {
      refetch()
  ...
  const addLineMutation = useMutation({
    mutationFn: (payload: CreateStockAdjustLinePayload) =>
      editLineId ? stockAdjustApi.updateLine(adjId, editLineId, payload) : stockAdjustApi.addLine(adjId, payload),
    onSuccess: () => {
      refetch()
  ...
  const deleteLineMutation = useMutation({
    mutationFn: (lineId: number) => stockAdjustApi.deleteLine(adjId, lineId),
  ...
  if (!adj) return <div className="flex items-center justify-center h-full text-gray-400">Not found</div>

  const isDraft = adj.status === 'DRAFT'
  const canConfirm = adj.status === 'APPROVED' || adj.status === 'SUBMITTED' || (isDraft && !adj.requireApproval)
  const canCancel = !['CONFIRMED', 'CANCELLED'].includes(adj.status)

  return (
    <div className="max-w-5xl mx-auto">
      {/* Top bar */}
      <div className="flex items-center gap-3 mb-5">
        <button onClick={() => navigate('/stock-adjust')}
          className="text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1">
          ← {t('common.back')}
        </button>
        <span className="text-gray-300">/</span>
        <h1 className="text-lg font-bold text-gray-800">{t('stockAdjust.title')}: {adj.adjustNo}</h1>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4 grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
        <div>
          <p className="text-xs text-gray-400 mb-0.5">{t('stockAdjust.location')}</p>
          <p className="font-semibold text-gray-800">{adj.location.locationName}</p>
          <p className="text-xs text-gray-400">{adj.location.locationCode}</p>
        </div>
        <div>
          <p className="text-xs text-gray-400 mb-0.5">{t('stockAdjust.adjustDate')}</p>
          <p className="font-medium text-gray-700">{adj.adjustDate.slice(0, 10)}</p>
        </div>
        <div>
          <p className="text-xs text-gray-400 mb-0.5">{t('stockAdjust.adjustType')}</p>
          <p className="font-medium text-gray-700">{adj.adjustType?.name ?? `ID: ${adj.adjustTypeId}`}</p>
        </div>
        {adj.adjustReason && (
          <div>
            <p className="text-xs text-gray-400 mb-0.5">{t('stockAdjust.adjustReason')}</p>
            <p className="text-gray-600">{adj.adjustReason}</p>
          </div>
        )}
        <div>
          <p className="text-xs text-gray-400 mb-0.5">{t('stockAdjust.requireApproval')}</p>
          <p className="font-medium text-gray-700">{adj.requireApproval ? 'Yes' : 'No'}</p>
        </div>
        {adj.remarks && (
          <div>
            <p className="text-xs text-gray-400 mb-0.5">{t('common.remarks')}</p>
            <p className="text-gray-600">{adj.remarks}</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-semibold text-gray-700 text-sm">{t('stockAdjust.lines')} ({adj.lines?.length ?? 0})</h2>
          {isDraft && (
            <button onClick={() => { setLineForm({ lineNo: (adj.lines?.length ?? 0) + 1, productId: 0, uomId: 0, qty: 1 }); setEditLineId(null); setShowAddLine(true) }}
              className="bg-blue-500 hover:bg-blue-600 text-white text-xs font-medium px-3 py-1.5 rounded-lg transition">
              + {t('stockAdjust.addLine')}
            </button>
          )}
        </div>
        ...
        {showAddLine && isDraft && (
          <div className="px-5 py-4 bg-blue-50 border-b border-blue-100">
            <div className="grid grid-cols-5 gap-3 items-end">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('stockAdjust.qty')} *</label>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-500 text-left">
            <tr>
              ...
              <th className="px-4 py-2 font-medium text-right">{t('stockAdjust.qty')}</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {!adj.lines?.length ? (
              ...
              <tr key={line.id} className={`hover:bg-gray-50 ${editLineId === line.id ? 'bg-blue-50' : ''}`}>
                ...
                <td className={`px-4 py-2 text-right font-semibold ${line.qty >= 0 ? 'text-green-700' : 'text-red-600'}`}>
                  {Number(line.qty) >= 0 ? '+' : ''}{Number(line.qty).toLocaleString()}
                </td>
