...
import type { CreatePurchaseOrderLinePayload } from '../types/purchase-order.types'
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4 grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
        ...
        <div>
          ...
          <p className="font-bold text-gray-800 text-base">{Number(po.poAmt).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
        </div>
        <div>
          ...
          <tbody className="divide-y divide-gray-100">
            {!po.lines?.length ? (
              ...
              <tr key={line.id} className={`hover:bg-gray-50 ${editLineId === line.id ? 'bg-blue-50' : ''}`}>
                ...
                <td className="px-4 py-2 text-right text-gray-700">{Number(line.qty).toLocaleString()}</td>
                <td className="px-4 py-2 text-right text-gray-700">{Number(line.price).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td className="px-4 py-2 text-right font-semibold text-gray-800">{Number(line.amount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
            ...
            <tfoot className="bg-gray-50">
              <tr>
                ...
                <td className="px-4 py-2 text-right font-bold text-gray-800">
                  {(po.lines ?? []).reduce((s, l) => s + Number(l.amount), 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </td>
