...
import { vendorApi } from '@/features/vendor/vendor.api'
import { locationApi } from '@/features/location/location.api'
import type { PurchaseOrder, CreatePurchaseOrderPayload } from './purchase-order.types'
...
function NewPoPanel({ onClose, onToast }: {
  ...
  const isValid = form.poNo.trim() && form.poDate && form.vendorId > 0 && form.locationId > 0
      ...
      <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input className="input" value={form.poNo}
              onChange={(e) => setForm(f => ({ ...f, poNo: e.target.value.toUpperCase() }))}
              ...
              {isLoading ? (
                ...
                <tr key={po.id} onClick={() => navigate(`/purchase-order/${po.id}`)}
                  ...
                  <td className="px-5 py-3 text-right font-medium text-gray-700">
                    {po.poAmt > 0 ? po.poAmt.toLocaleString(undefined, { minimumFractionDigits: 2 }) : '-'}
                  </td>
