...
import { productBrandApi } from '../services/product-brand.service'
import type { ProductBrand, CreateProductBrandPayload } from '../types/product-brand.types'
...
function BrandPanel({
  ...
  const [form, setForm] = useState<CreateProductBrandPayload>({
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      const payload = clean(form) as CreateProductBrandPayload
      return isEdit ? productBrandApi.update(brand!.id, payload) : productBrandApi.create(payload)
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => productBrandApi.delete(brand!.id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['product-brands'] })
    ...
    <div className="w-96 bg-white border-l border-gray-200 flex flex-col h-full shadow-xl">
      <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between bg-white">
        <h2 className="font-bold text-gray-800 text-sm">
          {isEdit ? t('productBrand.edit') : t('productBrand.add')}
        </h2>
      ...
      <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">{t('productBrand.code')} *</label>
          <input className="input" value={form.brandCode}
            onChange={(e) => setForm(f => ({ ...f, brandCode: e.target.value.toUpperCase() }))}
            disabled={isEdit} placeholder="NIKE" />
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">{t('productBrand.name')} *</label>
        ...
        {!confirmDelete ? (
          <>
            <button onClick={() => saveMutation.mutate()}
              disabled={saveMutation.isPending || !form.brandCode || !form.brandName}
...
export default function ProductBrandPage() {
  ...
  const [panel, setPanel] = useState<{ open: boolean; brand: ProductBrand | null }>({ open: false, brand: null })
  const [toast, setToast] = useState<{ message: string; type: 'success' | 'error' } | null>(null)

  const { data, isLoading } = useQuery({
    queryKey: ['product-brands'],
    queryFn: () => productBrandApi.getAll({ limit: 100 }),
  })
  ...
  const brands = useMemo(() => {
    ...
    return allBrands.filter((b) =>
      [b.brandCode, b.brandName].some((v) => v && String(v).toLowerCase().includes(q)),
    )
  }, [allBrands, search])
    ...
    <div className="flex h-full -m-6">
      <div className="flex flex-col flex-1 min-w-0">
        <div className="bg-gradient-to-r from-slate-800 via-slate-700 to-slate-600 px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-white font-bold text-lg">{t('productBrand.title')}</h1>
            <p className="text-slate-300 text-xs mt-0.5">{brands.length} / {allBrands.length} {t('common.items')}</p>
          </div>
          <button onClick={() => setPanel({ open: true, brand: null })}
        ...
        <div className="flex-1 overflow-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-500 text-left sticky top-0">
              <tr>
                <th className="px-5 py-3 font-medium">{t('productBrand.code')}</th>
                <th className="px-5 py-3 font-medium">{t('productBrand.name')}</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr key={b.id} onClick={() => setPanel({ open: true, brand: b })}
                  className={`cursor-pointer transition-colors hover:bg-blue-50 ${panel.brand?.id === b.id ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''}`}>
                  <td className="px-5 py-3 font-mono font-semibold text-slate-700">{b.brandCode}</td>
                  <td className="px-5 py-3 font-medium text-gray-800">{b.brandName}</td>
                  <td className="px-5 py-3 text-gray-500">{b.remarks ?? '-'}</td>
                  <td className="px-5 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${b.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-500'}`}>
      ...
      {panel.open && (
        <BrandPanel brand={panel.brand}
          onClose={() => setPanel({ open: false, brand: null })} onToast={showToast} />
      )}
