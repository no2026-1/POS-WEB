...
import { posConfigApi } from '../services/pos-config.service'
...
import type { PosConfig, CreatePosConfigPayload } from '../types/pos-config.types'
...
function PosConfigPanel({ config, onClose, onToast }: {
  ...
  const [form, setForm] = useState<CreatePosConfigPayload>({
    configCode: config?.configCode ?? genDocNo('POS'),
      ...
      <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            {isEdit ? (
              <input className="input" value={form.configCode} disabled />
            ) : (
              <div className="input bg-gray-50 text-gray-500 font-mono text-sm">{form.configCode}</div>
            )}
          </div>
          <div>
