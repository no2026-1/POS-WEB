...
import { basicApi } from '../services/basic.service'
import type {
  Basic,
  BasicLine,
  CreateBasicPayload,
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className="input" value={form.code}
                onChange={(e) => setForm(f => ({ ...f, code: e.target.value.toUpperCase() }))}
                disabled={isEdit} placeholder="USER_ROLE" />
            </div>
            <div>
            ...
            <div>
              ...
              <input className="input" value={form.nameTh}
                onChange={(e) => setForm(f => ({ ...f, nameTh: e.target.value }))}
        ...
        {isEdit && (
          <div className="flex flex-col flex-1">
            <div className="px-5 py-3 flex items-center justify-between bg-gray-50 border-b border-gray-100">
              ...
              <button onClick={() => setEditingLine('new')}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y divide-gray-100">
                  {editingLine === 'new' && (
                    <LineForm basicId={basic!.id} line={null}
                      onDone={() => setEditingLine(null)} onToast={onToast} />
                  )}
                  {lines.map((line) =>
                    editingLine && (editingLine as BasicLine).id === line.id ? (
                      <LineForm key={line.id} basicId={basic!.id} line={line}
                        onDone={() => setEditingLine(null)} onToast={onToast} />
                    ) : (
                      <tr key={line.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2 text-right">
                          <button onClick={() => setEditingLine(line)} className="text-blue-500 hover:text-blue-700 mr-2">✎</button>
                          <button onClick={() => deleteLineMutation.mutate(line.id)} className="text-red-400 hover:text-red-600">✕</button>
                        </td>
                      </tr>
                    )
                  )}
                  {lines.length === 0 && editingLine !== 'new' && (
                    <tr><td colSpan={5} className="text-center py-6 text-gray-400">{t('common.noData')}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
    ...
    <div className="flex h-full -m-6">
      <div className="flex flex-col flex-1 min-w-0">
        <div className="bg-gradient-to-r from-slate-800 via-slate-700 to-slate-600 px-6 py-4 flex items-center justify-between">
          ...
          <button onClick={() => setPanel({ open: true, basic: null })}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr key={b.id} onClick={() => setPanel({ open: true, basic: b })}
