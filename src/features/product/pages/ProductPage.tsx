...
import { productCategoryApi } from '@/features/product-category/services/product-category.service'
import { productBrandApi } from '@/features/product-brand/services/product-brand.service'
import { productGroupApi } from '@/features/product-group/services/product-group.service'
...
function ProductPanel({
  ...
  const [form, setForm] = useState<CreateProductPayload>({
    productCode: product?.productCode ?? genDocNo('PRD'),
    productName: product?.productName ?? '',
    uomId: product?.uomId ?? undefined as any,
      ...
      <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            {isEdit ? (
              <input className="input" value={form.productCode} disabled />
            ) : (
              <div className="input bg-gray-50 text-gray-500 font-mono text-sm">{form.productCode}</div>
            )}
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">{t('product.uom')} *</label>
            <select className="input" value={form.uomId || ''}
              onChange={(e) => setForm(f => ({ ...f, uomId: e.target.value ? Number(e.target.value) : undefined as any }))}>
