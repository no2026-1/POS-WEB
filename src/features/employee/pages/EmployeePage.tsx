...
import { departmentApi } from '@/features/department/services/department.service'
...
function EmployeePanel({
  ...
  const [form, setForm] = useState<CreateEmployeePayload>({
    empNo: employee?.empNo ?? genDocNo('EMP'),
      ...
      <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            {isEdit ? (
              <input className="input" value={form.empNo} disabled />
            ) : (
              <div className="input bg-gray-50 text-gray-500 font-mono text-sm">{form.empNo}</div>
            )}
          </div>
          <div>
