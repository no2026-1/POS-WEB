...
import { goodsReceiveApi } from './goods-receive.api'
import { locationApi } from '@/features/location/location.api'
import type { GoodsReceive, CreateGoodsReceivePayload } from './goods-receive.types'
...
function GrPanel({ gr, onClose, onToast }: {
  ...
  const isValid = form.grNo.trim() && form.grDate && form.purchaseOrderId > 0 && form.locationId > 0
      ...
      <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input className="input" value={form.grNo}
              onChange={(e) => setForm(f => ({ ...f, grNo: e.target.value.toUpperCase() }))}
              disabled={isEdit} placeholder="GR2024001" />
          </div>
          <div>
        ...
        <div>
          ...
          <input type="number" className="input" value={form.purchaseOrderId || ''}
            onChange={(e) => setForm(f => ({ ...f, purchaseOrderId: Number(e.target.value) }))}
            disabled={isEdit} placeholder="PO ID" />
          <p className="text-xs text-gray-400 mt-0.5">{t('goodsReceive.poIdHint')}</p>
        </div>

        {isEdit && gr?.purchaseOrder && (
          <div className="bg-blue-50 rounded-lg px-3 py-2 text-xs text-blue-700">
            PO: {gr.purchaseOrder.poNo} ({gr.purchaseOrder.status})
          </div>
        )}
              ...
              {isLoading ? (
                ...
                <tr key={gr.id} onClick={() => setPanel({ open: true, gr })}
                  ...
                  <td className="px-5 py-3 text-right font-medium text-gray-700">
                    {gr.grAmt > 0 ? gr.grAmt.toLocaleString(undefined, { minimumFractionDigits: 2 }) : '-'}
                  </td>
